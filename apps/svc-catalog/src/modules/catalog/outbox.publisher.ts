...
export class OutboxPublisher implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit() {
    // dispara um loop simples de pooling; pode ser trocado por @nestjs/schedule no futuro
    ...
    try {
      ...
      for (const item of items) {
        try {
          ...
          // marca como FAILED para evitar loop infinito; um reprocessador pode tratar depois
          await this.prisma.outbox.update({
            where: { id: item.id },
            data: { status: 'FAILED' },
          });
